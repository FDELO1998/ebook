...
function createGuessYouLike(data) {
    ...
    switch (n) {
        case 1:
        data['result'] = data.id % 2 === 0 ? '《Executing Magic》' : '《奇迹之书》'
        break
        case 2:
        data['result'] = data.id % 2 === 0 ? '《improving》' : '《梦想之书》'
        break
        case 3:
        data['result'] = data.id % 2 === 0 ? '《生活治理》' : '《爱情之书》'
...
app.get('/book/login', (req, res) => {
    const text = req.query.text
    const password = req.query.password
    const type = req.query.type
    const conn = connect()
    if (type ==='1'){
    const sql = `select * from login where text='${text}'`
    // const sql2 = `select * from login where text='${text}' and password = '${password}'`
    conn.query(sql, (err, results) => {
        if (err) {
            res.json({ error_code: 3, msg: '该用户名不存在，请注册'})
        } else {
            if (results[0].password == password.toString()) {
                 res.json({ type: 200, msg: '登录成功'})
             } else {
                res.json({ error_code: 3, msg: '账号或密码错误'})
             }
    ...
    const sql3 = `INSERT INTO login(text, password) VALUES('${text}','${password}')`
    conn.query(sql3, (err, results) => {
       if (err) {
        res.json({ error_code : 4, msg : '用户名已存在'})
       } else {
        res.json({ type : 200, msg : '注册成功'})
       }
    })
}
...
app.get('/book/home', (req, res) => {
    ...
    (err, results) => {
        ...
        const guessYouLike = []
        const banner = constant.resUrl + 'fonts/home_banner.jpg'
        const recommend = []
        const featured = []
        const random = []
